...
resource "aws_lb" "alb" {
    ...
    subnets            = [data.aws_subnet.public_prod_subnet_az1.id, data.aws_subnet.public_prod_subnet_az2.id]
...
resource "aws_lb_target_group_attachment" "blue_group_attachment_az1" {
    ...
    target_id        = data.aws_instance.prod_instance_az1.id
    port             = 80
}
...
resource "aws_lb_target_group_attachment" "green_group_attachment_az2" {
    ...
    target_id        = data.aws_instance.prod_instance_az2.id
    port             = 80
}
